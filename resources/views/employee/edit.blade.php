...
@section('content')
    {{-- ===== HEADER ===== --}}
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-2">Update Employee Data</h1>
        <p class="text-gray-500">Manage employee information and their complete details</p>
    </div>

    {{-- ===== FORM SECTION ===== --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('karyawan.update', $employee->id) }}" method="POST" class="space-y-6">
            @method('PUT')
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Full Name</label>
                    <input name="name" type="text" id="name" value="{{ old('name', $employee->name) }}"
                ...
                <div>
                    ...
                    <input name="nip" type="text" id="nip" value="{{ old('nip', $employee->nip) }}"
                        class="mt-1 block w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('nip')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input name="position" type="text" id="position" value="{{ old('position', $employee->position) }}"
                        class="mt-1 block w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('position')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input name="salary" type="number" id="salary" value="{{ old('salary', $employee->salary) }}"
                        class="mt-1 block w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('salary')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input name="join_date" type="date" id="join_date" value="{{ old('join_date', $employee->join_date) }}"
                        class="mt-1 block w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('join_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
